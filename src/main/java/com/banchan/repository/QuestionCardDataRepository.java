...
public interface QuestionCardDataRepository extends JpaRepository<QuestionCardData, Long> {
    ...
    @Query(
            value = "SELECT qu.*, qd.id detail_id, qd.type detail_type, qd.content detail_content FROM (  " +
                    ...
                    "      SELECT q.*  " +
                    "      FROM questions q  " +
                    "      LEFT JOIN (SELECT DISTINCT question_id FROM votes WHERE user_id = :userId) v  " +
    ...
    @Query(
            value = "SELECT qu.*, qd.id detail_id, qd.type detail_type, qd.content detail_content " +
                    ...
                    "      FROM questions q " +
                    "      JOIN (SELECT question_id, vote_time FROM votes WHERE user_id = :userId GROUP BY question_id) v " +
                    "        ON q.id = v.question_id " +
    ...
    @Query(
            value = "SELECT qu.*, qd.id detail_id, qd.type detail_type, qd.content detail_content " +
                    ...
                    "    LEFT JOIN votes v " +
                    "      ON q.id = v.question_id " +
                    "    WHERE q.user_id = :userId AND q.report_state = 0 " +
                    "    GROUP BY q.id " +
                    "    ORDER BY q.decision IS NULL DESC, q.id DESC " +
                    "    LIMIT :start, :counting " +
                    "  ) q " +
                    "  LEFT JOIN reviews r " +
                    "    ON q.id = r.question_id " +
                    "  LEFT JOIN usernames u " +
